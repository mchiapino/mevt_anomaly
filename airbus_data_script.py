...
def theta_constraint(theta):
    ...
    new_theta = mc.rho_nu_to_theta(new_rho, nu)
...
data_rank = extr.rank_transformation(data_doubled)
...
alphas_c = ga.alphas_complement(alphas_converted, dim)
...
points_face = dr.assign_face_to_points(X_damex, alphas_converted, [])
...
means_emp, weights_emp = dr.estimates_means_weights(X_extr,
                                                    X_damex,
                                                    alphas_converted, [])
rho_emp = mc.means_weights_to_rho(means_emp, weights_emp, alphas_converted)

# Means and weights that verify moment constraint
rho_0 = mc.project_rho(rho_emp, dim)
...
# EM algorithm
theta = mc.rho_nu_to_theta(rho_emp, nu)
...
for k in range(n_loop):
    ...
    gamma_z = em.compute_gamma_z(X_extr, alphas_converted, [],
                                 theta, rho_0, lambd)


weights_gam = np.mean(gamma_z, axis=0)
means_res, weights_res = mc.rho_to_means_weights(rho)
...
for x in X_extr:
    ...
    for k, alpha in enumerate(alphas_converted):
        ...
        w = x[alpha] / r
        eps_noise = x[alphas_c[k]]
        f_x_alpha = (dr.dirichlet(w, means_res[k], nu[k]) *
                     np.prod(dr.exp_distrib(eps_noise, lambd)) * r**-2)
...
# , node_color=points_face/float(K), with_label=True)
# plt.show()
